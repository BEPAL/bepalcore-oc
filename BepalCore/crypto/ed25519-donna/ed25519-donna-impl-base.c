...
void ge25519_encode(ge25519 res, unsigned char pk[32]) {
    bignum25519 x, y, z, temp;
    curve25519_mul(x,res.x,res.t);
    curve25519_mul(y,res.y,res.z);
    curve25519_mul(z,res.z,res.t);
    ge25519_invert(temp, z);
    curve25519_mul(x, x, temp);
    curve25519_mul(y, y, temp);
    curve25519_contract(pk, y);
    int bo = (x[0] & 1) != 0;
    pk[31] |= (bo ? 0x80 : 0);
}

void ge25519_invert(bignum25519 outvalue, bignum25519 invalue) {
    bignum25519 f0, f1;
    // 2 == 2 * 1
    curve25519_square(f0, invalue);
    // 9 == 9
    pow2to9(f1, invalue);
    // 11 == 9 + 2
    curve25519_mul(f0, f0, f1);
    // 2^252 - 2^2
    curve25519_pow_two252m4(f1, invalue);
    // 2^255 - 2^5
    for (int i = 1; i < 4; ++i) {
        curve25519_square(f1, f1);
    }
    // 2^255 - 21
    curve25519_mul(outvalue, f0, f1);
}

int ge25519_decode(ge25519 *r, const unsigned char p[32]) {
    const unsigned char zero[32] = {0};
    const bignum25519 one = {1};
    unsigned char parity = p[31] >> 7;
    unsigned char check[32];
...
//    bignum25519 v;
    ...
    // u = y^2 - 1
    curve25519_square(ysquare, r->y); /* x = y^2 */
    curve25519_sub(num, ysquare, one);
    
    // v = d * y^2 + 1
    curve25519_mul(den, ysquare, ge25519_ecd);
    curve25519_add(den, den, one);
