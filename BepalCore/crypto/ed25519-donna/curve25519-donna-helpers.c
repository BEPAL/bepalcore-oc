 ...
 * z^((p-5)/8) = z^(2^252 - 4)
 */
void curve25519_pow_two252m4(bignum25519 two252m3, const bignum25519 z) {
    bignum25519 ALIGN(16) b,c,t0;
    ...
    /* 2^5 - 2^0 = 31 */ curve25519_mul_noinline(b, t0, b);
    /* 2^250 - 2^0 */ curve25519_pow_two5mtwo0_two250mtwo0(b);
    /* 2^252 - 2^2 */ curve25519_square_times(two252m3, b, 2);
}


void pow2to9(bignum25519 outvalue,bignum25519 value) {
    bignum25519 f;
    curve25519_square(f, value);
    curve25519_square(f, f);
    curve25519_square(f, f);
    curve25519_mul(outvalue, value, f);
}
